...
export function adbutler(global,data){
    var placeholderID;
    validateData(data, ['mid', 'zone_id', 'width', 'height'], ['kw', 'place']);

    data['place'] = data['place'] || 0;
        
    placeholderID = 'placement_' + data['zone_id'] + '_' + data['place'];
    ...
    global.AdButler.ads.push({
        handler: function (opt) {
            global.AdButler.register(data['mid'], data['zone_id'], [data['width'], data['height']], placeholderID, opt);
        },
        opt: {
            place: data['place'],
